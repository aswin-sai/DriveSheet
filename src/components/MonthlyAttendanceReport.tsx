...
import { Calendar, Download, Filter, Search, CheckCircle, XCircle, Clock, Grid3X3, ArrowLeft } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 mb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <label className="text-base font-medium text-slate-700 dark:text-slate-300 flex-shrink-0">
            Select Month:
          </label>
          <input
            type="month"
            ...
            className="px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base"
          />
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Total Present</p>
              <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{stats.totalPresent}</p>
            </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Total Absent</p>
              <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{stats.totalAbsent}</p>
            </div>
      ...
      <Card className="p-4 mb-6">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-3">Quick Summary</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
            <span className="text-slate-600 dark:text-slate-400">Best Attendance:</span>
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
            <span className="text-slate-600 dark:text-slate-400">Most Absent:</span>
            <span className="font-medium text-slate-900 dark:text-slate-100">
              {filteredData.length > 0 ? 
                filteredData.reduce((most, current) => 
                  current.absentDays > most.absentDays ? current : most
                ).student.name : 'N/A'
              }
            </span>
          </div>
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
            ...
            <span className="font-medium text-slate-900 dark:text-slate-100">
              {filteredData.reduce((total, data) => total + data.student.totalSessions, 0)}
            </span>
          </div>
        </div>
      </Card>
        ...
        <div className="block lg:hidden">
          <div className="space-y-4">
            {filteredData.map((data, index) => (
              <div key={data.student.id} className={`border border-slate-200 dark:border-slate-700 rounded-lg p-4 ${index % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-50 dark:bg-slate-800'}`}>
                ...
                <div className="grid grid-cols-7 gap-px mb-2 bg-slate-200 dark:bg-slate-700 rounded-lg p-1">
                  {datesInMonth.slice(0, 7).map((date) => (
                    <div key={date} className="text-center p-1 bg-white dark:bg-slate-800 rounded">
                      <div className="text-xs text-slate-500 mb-1">
                        {new Date(date).getDate()}
                      </div>
                      <div
                        className={`w-3 h-3 rounded-sm mx-auto ${getStatusColor(data.attendanceByDate[date])}`}
